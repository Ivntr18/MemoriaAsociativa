...
while True:
    sel=int(input('Choose how many numbers will be write in each memory registrer (1, 2): '))
    if sel==1 or sel==2:
        break
...
np.savetxt('main_average_precision--{0}.npy'.format(sel), main_average_precision, delimiter=',')
np.savetxt('main_average_recall--{0}.npy'.format(sel), main_average_recall, delimiter=',')
np.savetxt('main_average_entropy--{0}.npy'.format(sel), main_average_entropy, delimiter=',')
